...
```bash
sample-step-functions$ sam build
```
...
```bash
sample-step-functions$ sam logs -n StockCheckerFunction --stack-name sample-step-functions --tail
```
...
```bash
sample-step-functions$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name sample-step-functions
```
